...
This repo includes a GitHub Actions workflow that builds the Land app from `apps/land` and produces unsigned macOS artifacts on `macos-latest`.

### Manual build

1. Go to GitHub -> Actions -> build-mac.
2. Click Run workflow.
3. Download `Tamagotchi-Land-Pets-mac-arm64` and `Tamagotchi-Land-Pets-mac-x64` from the run summary.
